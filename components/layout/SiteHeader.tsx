...
export default function SiteHeader() {
  ...
  return (
    <header className="w-full bg-brand-black border-b-[3px] border-brand-red px-4 py-3">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        {/* Logo + tagline — top row on mobile, left side on sm+ */}
        ...
        {/* Buy CTA + Wallet — bottom row on mobile, right side on sm+ */}
        <div className="flex items-center gap-3">
          {mint && ticker && (
            <a
              ...
              className="font-display uppercase tracking-widest text-xs bg-brand-yellow text-brand-black px-3 py-1 hover:bg-yellow-300 transition-colors font-bold whitespace-nowrap"
            >
              BUY ${ticker}
            </a>
          )}
